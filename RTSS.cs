    ...
    public class RTSSSM
    {
        ...
        public RTSSSM Update()
        {
            ...
            try
            {
                ...
                //Forms has erorr since this is thread, dispite being in a try catch wont even skip this for no reason. So have to have a null check.
                if (rtssStream != null && (rtssStream.CanRead && rtssStream.Capacity >= headerBufferSize))
                {
                    unsafe
                    {
                ...
                {
                    //Logger queue, Error reading RTSS memory. RTSS is Null.
                }
            }
            catch (Exception ex)
            {
